...
# Solver for http://projecteuler.net/problem=61
class Problem61 < Problem
  def solve # rubocop:disable MethodLength, CyclomaticComplexity
    ...
    found.keys.reject { |a| sets.map { |i| i & a }.sort.uniq.size != 6 }.
      flatten.reduce(:+)
  end
  ...
  def valid?(route) # rubocop:disable CyclomaticComplexity
...
puts Problem61.solution if $PROGRAM_NAME == __FILE__
